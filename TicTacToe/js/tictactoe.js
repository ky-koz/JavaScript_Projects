...
function btnDisabled(btn) {
    btn.style.color = "#fff";
    btn.style.border = "2px solid rgb(152, 152, 102)";
...
function stopGame() {
    ...
    // this clearss the running log of all game moves
...
// proposed avatar assignments and prevents them being the same.
...
// as squaress are selected they check in with this function to see if that particular
...
function checkForWinCon() {
    ...
    //console.log("New CHECK: "+document.getElementId('gameMsg').innerHTML);
    check4Tie();
}
...
function winner(winDetected,winCon) {
    if (winDetected == "win") {
        ...
        glowBoard(winCon); // call function to make the gameboard pulse colors
    }
}
...
// these functions will produce game sounds depending on the occassion
...
function square1Animate() {
    ...
    if (activePlayer != "Game Stopped") { //if game has not yet started prevent avatar placement
        ...
        if (verdict == undefined) { // if verdict is empty than the square is unoccupied.
            var paintAvatar = determineAvatar(); // if verdict is empty than the square is unoccupied.
            ...
            squareSound(); // play a game sound when the avatar is place
        }
    }
}
